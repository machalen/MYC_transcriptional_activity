...
4. MYC transcriptional activity inferred from three complementary approaches:
   ...
   * Gene Set Variation Analysis (GSVA) with the `GSVA` package on the log<sub>2</sub>-CPM matrix.
   * Transcription-factor activity inference with the Univariate Linear Model (ULM) implemented in `decoupleR`, using the CollecTRI regulon.
